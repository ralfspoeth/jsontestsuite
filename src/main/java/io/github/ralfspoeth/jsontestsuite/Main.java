                ...
                .forEach(p -> {
                    try {
                        ...
                        if (nMatcher.matches(p.getFileName())) {
                            System.out.printf("%s shouldn't have been parsed into %s%n", p.getFileName(), result);
                        } else {
                            System.out.printf("%s successfully parsed into %s%n", p.getFileName(), result);
                        }
                    } catch (Throwable t) {
                        if (yMatcher.matches(p.getFileName())) {
                            System.out.printf("%s should haven been successfully parsed; exception is: %s%n", p.getFileName(), t);
